...
@Slf4j
@Singleton
class ChocoGrinderOverlay extends OverlayPanel
{
	private final Client client;
	private final ChocoGrinderPlugin plugin;
	private final ChocoGrinderConfig config;

	String timeFormat;
	private String infoStatus = "Starting...";

	@Inject
	private ChocoGrinderOverlay(final Client client, final ChocoGrinderPlugin plugin, final ChocoGrinderConfig config)
	{
		super(plugin);
		...
		this.plugin = plugin;
		this.config = config;
		getMenuEntries().add(new OverlayMenuEntry(RUNELITE_OVERLAY_CONFIG, OPTION_CONFIGURE, "Choco Grinder overlay"));
	}
	...
	public Dimension render(Graphics2D graphics)
	{
		...
		tableDelayComponent.addRow("Sleep delay:", ChocoGrinderPlugin.sleepLength + "ms");
